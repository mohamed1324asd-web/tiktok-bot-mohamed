...
@app.route('/')
def home(): return "✅ Bot is Live with Multi-Channel System!"
...
async def start_cmd(message: types.Message):
    ...
    if message.from_user.id == ADMIN_ID:
        await message.reply("أهلاً بك يا مطورنا! لوحة التحكم جاهزة.", reply_markup=admin_main_keyboard())
    elif not is_sub:
        markup = InlineKeyboardMarkup(row_width=1)
        for ch in channels:
        ...
        await message.reply("⚠️ عذراً! يجب الاشتراك في القنوات التالية أولاً:", reply_markup=markup)
        return
    else:
        await message.reply("<b>مرحباً بك في بوت التحميل! 🎬</b>\n\nأرسل رابط الفيديو للبدء.\n\n<i>المطور: @i_wi_w</i>", parse_mode='HTML')

# --- معالجة لوحة تحكم القنوات ---
...
async def admin_callbacks(c: types.CallbackQuery):
    ...
    if c.data == "toggle_all_status":
        ...
        await c.message.answer("أرسل يوزر القناة الجديد (مثال: @my_channel)")
...
async def add_channel_confirm(m: types.Message):
    ...
    if m.text not in config["channels"]:
        ...
        await m.reply(f"✅ تم إضافة {m.text}", reply_markup=admin_main_keyboard())
...
async def ask_br(m: types.Message): await m.reply("أرسل نص الإذاعة الآن:")
...
async def check_btn(c: types.CallbackQuery):
    ...
    if is_sub: await c.message.edit_text("✅ تم التحقق! أرسل الرابط الآن.")
